...
//		gtk_tree_store_set (treestore, &toplevel,
		...
		Chain_COLUMN,gMyInfo.MyPatchInfo[Loop].Chain,
		-1);
	}
...
GtkTreeModel *create_combo_model(void) {
   ...
   for(i = 0; i < MaxCommands; i++) {
      ...
      gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, CustomCommands[i], -1);
   }
...
//	g_signal_connect(renderer, "toggled", (GCallback) PatchListEdited, view);
	...
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Name_COLUMN));   
	g_signal_connect(G_OBJECT(renderer),"editing-started",G_CALLBACK(text_editing_started),Button_COLUMN);  

  /* --- Bank_COLUMN --- */
 renderer = gtk_cell_renderer_text_new ();
  g_object_set(renderer, "editable", TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (view),
		Bank_COLUMN, "Bank", renderer,
		"text", Bank_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Bank_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
 ...
 renderer = gtk_cell_renderer_text_new ();
  g_object_set(renderer, "editable", TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (view),
		Patch_COLUMN, "Patch", renderer,
		"text", Patch_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Patch_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
 ...
 renderer = gtk_cell_renderer_text_new ();
  g_object_set(renderer, "editable", TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (view),
		Output_COLUMN, "Port", renderer,
		"text", Output_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Output_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
 ...
 renderer = gtk_cell_renderer_text_new ();
  g_object_set(renderer, "editable", TRUE, NULL);
  gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (view),
		Channel_COLUMN, "Channel", renderer,
		"text", Channel_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Channel_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
...
//  g_object_set(renderer, "editable", TRUE, NULL);
  g_object_set(renderer, "text-column", 0,
                  ...
                  NULL);
   gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (view),
		Command_COLUMN, "Command", renderer,
		"text", Command_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Command_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
...
//		"mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE,
		"text", Chain_COLUMN,
		NULL);
	g_object_set_data(G_OBJECT(renderer), "column", GUINT_TO_POINTER(Chain_COLUMN));   
	g_signal_connect(renderer, "edited", (GCallback) PatchListEdited, view);
