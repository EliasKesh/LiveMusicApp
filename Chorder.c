...
char	chMajor[] = 		{ 	0,	SN,	4,	SN,	7,	SN,	11, EOfscl };
char	chMinor[] = 		{	0,	SN,	3,	SN,	7,	SN,	10, EOfscl };
char	chDominant[] = 	{	0,	SN,	4,	SN,	7,	SN,	10, EOfscl };
char	chMajor6[] = 		{ 	0,	SN,	4,	SN,	7,	9,	SN, EOfscl };
char	chMinor6[] = 		{	0,	SN,	3,	SN,	7,	9,	SN, EOfscl };
char	chMajor9[] = 		{ 	0,	SN,	4,	SN,	7,	SN,	SN, SN, 14, EOfscl };
char	chMinor9[] = 		{	0,	SN,	3,	SN,	7,	SN,	SN, SN, 14, EOfscl };
char	chDom9[] = 		{	0,	SN,	4,	SN,	7,	SN,	10, SN, 14, EOfscl };
char	ch69[] = 			{	0,	SN,	4,	SN,	7,	SN,	10, SN, 14, EOfscl };
char	scMajor[] = 		{	0,	2, 	4,	5,	7,	9,	11, EOfscl	};
char	scMinor[] = 		{	0,	2, 	3,	5,	7,	8,	10, EOfscl	};
...
thePreChord 	pchMaj[] = {
    ...
    {  1, 1, 1, 0}, { 2, 3, 5, 2}, { 3, 4, 1, 2}, { 4, 2, 3, 1}, {-1,-1,-1,-1}
};
thePreChord 	pchMin[] = {
    /* S  Fi D Fr     S  Fi D Fr     S  Fi D Fr     S  Fi D Fr	*/
    {  1, 1, 1, 0}, { 2, 3, 5, 2}, { 3, 4, 1, 2}, { 4, 2, 3, 0}, {-1,-1,-1,-1}
};
...
thePreChord 	pchMaj7O[] = {
    /* S  Fi D Fr     S  Fi D Fr     S  Fi  D  Fr    S  Fi D  Fr	*/
    {  1, 1, 1, 0}, { 4, 3, 7, 1}, { 5, 4, 3, 3}, {-1,-1,-1,-1}
};
thePreChord 	pchMin7O[] = {
    /* S  Fi D Fr     S  Fi D Fr     S  Fi  D  Fr    S  Fi D  Fr	*/
    {  1, 1, 1, 0}, { 4, 3, 7, 0}, { 5, 4, 3, 2}, {-1,-1,-1,-1}
};
...
static gboolean draw_fretboard_background (GtkWidget *area, GdkEventExpose *event);
...
void 	SetupFretBoard(void) {
    ...
    for (StringLoop = 0; StringLoop < NumStrings; StringLoop++) {
        for (FretLoop = 1; FretLoop < MaxNumFrets; FretLoop++) {
            StringLayout[StringLoop][FretLoop] =
                GetIntervalNote(StringLayout[StringLoop][FretLoop-1],1);
        }
    }
}
...
//    char	NewNote;
     ...
     */
    Loop = 1;
    for (String = 0; String < NumStrings  && Loop; String++) {
...
//               myChord.ChordNotes[String][Fret] = Value;
    for (Loop = 0; myPreChord[Loop].Finger != -1; Loop++) {
        printf("C_String %d, C_Fret %d\n", myPreChord[Loop].String, myPreChord[Loop].Fret);
        myChord.ChordNotes[MaxNumStrings - (String + myPreChord[Loop].String)][Fret + myPreChord[Loop].Fret ] =
            myPreChord[Loop].Interval;
    }
...
* Description:
*---------------------------------------------------------------------*/
int ChorderMain(GtkWidget *window ) {
...
//	gtk_settings_set_property_value(RootCombo, "gtk-touchscreen-mode", &touchscreen_mode);
    ...
    for (Loop = 0; myChordMenu[Loop].ChordList ; Loop++) {
        gtk_combo_box_append_text(GTK_COMBO_BOX(ScaleCombo), myChordMenu[Loop].Name);
    }
...
*---------------------------------------------------------------------*/
gboolean
draw_fretboard_background(GtkWidget *widget, GdkEventExpose *event) {
    GError* error = NULL;
    int		Loop;
...
//	GdkColor labeloffcolor = { 0, 0xff <<8, 0xff <<8, 0xff <<8 };
    ...
    // Draw the ground wood.
    gdk_draw_pixbuf( widget->window, gc,  Fingerboard,
                     0, 0, XOffset, YOffset, FretOffset * (MaxDisplayFrets + 1) + 30, StringOffset * NumStrings + 30,
//                     0, 0, 0, 0, WindowWidth - 100, StringOffset * NumStrings + 30,
    ...
    for (Loop = 0; Loop <= MaxDisplayFrets; ++Loop ) {
        ...
        gdk_draw_line( widget->window, gc, XOffset+ 30 + Loop*FretOffset, YOffset+ 20, XOffset+ 30 + Loop*FretOffset, YOffset+262 );
        gdk_gc_set_rgb_fg_color( gc, &fretcolor2 );
        gdk_draw_line( widget->window, gc, XOffset+ 32 + Loop*FretOffset, YOffset+20, XOffset+ 32 + Loop*FretOffset, YOffset+262 );
        gdk_gc_set_rgb_fg_color( gc, &fretcolor3 );
        gdk_draw_line( widget->window, gc, XOffset+ 34 + Loop*FretOffset, YOffset+20, XOffset+ 34 + Loop*FretOffset, YOffset+262 );
...
//		gdk_gc_set_line_attributes(gc, 2, GDK_LINE_ON_OFF_DASH, GDK_CAP_ROUND, GDK_JOIN_MITER );
        gdk_draw_layout( widget->window, gc, XOffset+30 + Loop*FretOffset, YOffset+270 , layout );
        ...
        if (Loop != MaxDisplayFrets)
            ...
            case 11:
                gdk_draw_pixbuf( widget->window, gc, FingerboardDot,
                                 0, 0,
                                 XOffset+15+FretOffset/2 + Loop*FretOffset, StringOffset * (3*NumStrings/4) - 15+YOffset,
                                 -1, -1,
                                 GDK_RGB_DITHER_MAX,
                                 0, 0);
                gdk_draw_pixbuf( widget->window, gc, FingerboardDot,
                                 0, 0,
                                 XOffset+15+FretOffset/2 + Loop*FretOffset, StringOffset * NumStrings/4 + 15+YOffset,
                                 -1, -1,
                                 GDK_RGB_DITHER_MAX,
                                 0, 0);
                break;
    ...
    for (Loop = 0; Loop < NumStrings; ++Loop ) {
        ...
        gdk_draw_layout( widget->window, gc, XOffset + 5, YOffset + 12 + 30*Loop, layout );
        gdk_draw_layout( widget->window, gc, XOffset + 45 + FretOffset * MaxDisplayFrets, YOffset + 12 + 30*Loop, layout );
        gdk_draw_line( widget->window, gc, XOffset + 30, YOffset + 20 + Loop*StringOffset, XOffset +30 + FretOffset * MaxDisplayFrets , YOffset + 20 + Loop*StringOffset );

        gdk_gc_set_rgb_fg_color( gc, &stringcolor1 );
        gdk_draw_line( widget->window, gc, XOffset + 30, YOffset + 20 + Loop*StringOffset, XOffset + 30 + FretOffset * MaxDisplayFrets, YOffset + 20 + Loop*StringOffset );
        gdk_gc_set_rgb_fg_color( gc, &stringcolor2 );
        gdk_draw_line( widget->window, gc, XOffset + 30, YOffset + 22 + Loop*StringOffset, XOffset + 30 + FretOffset * MaxDisplayFrets, YOffset + 22 + Loop*StringOffset );
        gdk_gc_set_rgb_fg_color( gc, &stringcolor3 );
        gdk_draw_line( widget->window, gc, XOffset + 30, YOffset + 24 + Loop*StringOffset, XOffset + 30 + FretOffset * MaxDisplayFrets, YOffset + 24 + Loop*StringOffset );
    }
    ...
    for (StringLoop = 0; StringLoop < NumStrings; StringLoop++) {
        for (FretLoop = 0; FretLoop < MaxDisplayFrets; FretLoop++) {
            if (myChord.ChordNotes[StringLoop][FretLoop]) {
                 ...
                 */
                gdk_draw_pixbuf( widget->window, gc, theBall,
                                 0, 0,
                                 FretOffset/2 + 15 + FretOffset*(FretLoop) + XOffset, YOffset + StringOffset*StringLoop,
                                 -1, -1,
                                 GDK_RGB_DITHER_MAX, 0, 0);
                gdk_gc_set_rgb_fg_color( gc, &labeloffcolor );
                sprintf(StrBuf, "%d", myChord.ChordNotes[StringLoop][FretLoop]);
                pango_layout_set_text( layout, StrBuf, 2 );
                gdk_draw_layout( widget->window, gc,FretOffset/2 + 30 + FretOffset*(FretLoop) + XOffset,
